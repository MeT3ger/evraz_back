...
class TestZipPreproc(unittest.TestCase):
    ...
    def test_create_json(self):
        ...
        try:
            asyncio.run(self.test_file_1.dump_json(jsonname=FileLibrary.get_json_path(Files.anubis)))
        except Exception as e:
            self.fail(msg=f'Failed on Anunbis-develop.zip with {e}')

        #2 Тест с backend-master.zip
        try:   
            asyncio.run(self.test_file_2.dump_json(jsonname=FileLibrary.get_json_path(Files.backend_master)))
        except Exception as e:
            self.fail(msg=f'Failed on backend-master.zip with {e}')
        
        #3 Тест с donna-backend-master.zip
        try:
            asyncio.run(self.test_file_3.dump_json(jsonname=FileLibrary.get_json_path(Files.donna_backend_master)))
        except Exception as e:
            self.fail(msg=f'Failed on donna-backend-master.zip with {e}')
        
        #4 Тест с final-year-project-backend-api-master.zip
        try:
            asyncio.run(self.test_file_4.dump_json(jsonname=FileLibrary.get_json_path(Files.final_year)))
        except Exception as e:
            self.fail(msg=f'Failed on final-year-project-backend-api-master.zip with {e}')
        
        #5 Тест с Flask-PostgreSQL-API-Seed-master.zip
        try:
            asyncio.run(self.test_file_5.dump_json(jsonname=FileLibrary.get_json_path(Files.flask_postgres)))
        except Exception as e:
            self.fail(msg=f'Failed on Flask-PostgreSQL-API-Seed-master.zip with {e}')
        
        #6 Тест с FlaskApiEcommerce-master.zip
        try:
            asyncio.run(self.test_file_6.dump_json(jsonname=FileLibrary.get_json_path(Files.flask_api)))
        except Exception as e:
            self.fail(msg=f'Failed on Flask-PostgreSQL-API-Seed-master.zip with {e}')
        
        #7 Тест с gramps-web-api-master.zip
        try:
            asyncio.run(self.test_file_7.dump_json(jsonname=FileLibrary.get_json_path(Files.gramps_api)))
        except Exception as e:
            self.fail(msg=f'Failed on gramps-web-api-master.zip with {e}')
        
        #8 Тест с hackernews-api-main.zip
        try:
            asyncio.run(self.test_file_8.dump_json(jsonname=FileLibrary.get_json_path(Files.hacker_api)))
        except Exception as e:
            self.fail(msg=f'Failed on hackernews-api-main.zip with {e}')
        
        # 9 Тест с http-api.zip
        try:
            asyncio.run(self.test_file_9.dump_json(jsonname=FileLibrary.get_json_path(Files.http_api)))
        except Exception as e:
            self.fail(msg=f'Failed on http-api.zip with {e}')
        
        #10 Тест с luncher-api-master.zip
        try:
            asyncio.run(self.test_file_10.dump_json(jsonname=FileLibrary.get_json_path(Files.launcher_api)))
        except Exception as e:
            self.fail(msg=f'Failed on luncher-api-master.zip with {e}')
        
        #11 Тест с ml-flask-api-master.zip
        try:
            asyncio.run(self.test_file_11.dump_json(jsonname=FileLibrary.get_json_path(Files.ml_flask_api)))
        except Exception as e:
            self.fail(msg=f'Failed on ml-flask-api-master.zip with {e}')

        #12 Тест с RESTfulAPI-master.zip
        try:
            asyncio.run(self.test_file_12.dump_json(jsonname=FileLibrary.get_json_path(Files.rest_api)))
        except Exception as e:
...
'''            
    def test_find_file_types(self):
        ...
        #1 Тест с Anunbis-develop.zip
        res_func = asyncio.run(self.test_file_1._ZipPreproc__find_file_types())
        ...
        #2 Тест с backend-master.zip
        res_func = asyncio.run(self.test_file_2._ZipPreproc__find_file_types())
        ...
        #3 Тест с donna-backend-master.zip
        res_func = asyncio.run(self.test_file_3._ZipPreproc__find_file_types())
        ...
        #4 Тест с final-year-project-backend-api-master.zip
        res_func = asyncio.run(self.test_file_4._ZipPreproc__find_file_types())
        ...
        #5 Тест с Flask-PostgreSQL-API-Seed-master.zip
        res_func = asyncio.run(self.test_file_5._ZipPreproc__find_file_types())
        ...
        #6 Тест с FlaskApiEcommerce-master.zip
        res_func = asyncio.run(self.test_file_6._ZipPreproc__find_file_types())
        ...
        #7 Тест с gramps-web-api-master.zip
        res_func = asyncio.run(self.test_file_7._ZipPreproc__find_file_types())
        ...
        #8 Тест с hackernews-api-main.zip
        res_func = asyncio.run(self.test_file_8._ZipPreproc__find_file_types())
        ...
        #9 Тест с http-api.zip
        res_func = asyncio.run(self.test_file_9._ZipPreproc__find_file_types())
        ...
        #10 Тест с luncher-api-master.zip
        res_func = asyncio.run(self.test_file_10._ZipPreproc__find_file_types())
        ...
        #11 Тест с ml-flask-api-master.zip
        res_func = asyncio.run(self.test_file_11._ZipPreproc__find_file_types())
